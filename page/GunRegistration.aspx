<%@ Page Title="" Language="C#" MasterPageFile="~/master/main.master" AutoEventWireup="true" CodeFile="GunRegistration.aspx.cs" Inherits="master_Default" %>
                        ...
                        <table id="dynamic-table" class="table table-striped table-bordered table-hover"   >
                            ...
                            <tbody>
                                <tr ng-repeat="item in DATA" style="width: 100%">
                                    <td nowrap><a ng-click="fnopenImage(item.BookNo, item.PageNo, item.PageVersion, item.GR_ID)" style="cursor: pointer">ดูเอกสาร</a></td>
                                    <td nowrap>{{item.BookNo}}</td>
                                    <td nowrap>{{item.PageNo}}</td>
                                    <td nowrap>{{item.PageVersion}}</td>
        ...
        function StartPages($scope, $http) {
            ...
            $scope.fnSearchData = function () {
                if ($scope.dataCondition.GunRegID == "" && $scope.dataCondition.GunNo == ""
                    ...
                    && $scope.dataCondition.GunSize == "" && $scope.dataCondition.GunOwner == "" && $scope.dataCondition.GunRemark == "" && $scope.dataCondition.GunGroup=="") {
            ...
            $scope.fnopenImage = function (BookNo, PageNo, VersionPage, GunRegID) {
                var perId = getParamValue("ID")
                if (perId != "5")
                    openImage(BookNo, PageNo, VersionPage, perId);
                if (perId == "5")
                    openImageHide(BookNo, PageNo, VersionPage, GunRegID, perId);
            }
        }

        function openImage(BookNo, PageNo, VersionPage, perId) {
            window.open('../gunbook/showimage.aspx?Book=' + BookNo + '&Page=' + PageNo + '&Ver=' + VersionPage + '&perId=' + perId, '_blank');
        }
        function openImageHide(BookNo, PageNo, VersionPage, GunRegID, perId) {
            window.open('../gunbook/showimageHide.aspx?Book=' + BookNo + '&Page=' + PageNo + '&Ver=' + VersionPage + '&GunID=' + GunRegID + '&perId=' + perId, '_blank');
        }
