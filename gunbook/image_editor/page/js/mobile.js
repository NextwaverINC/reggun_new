...
var $btnDeleteFile = $('#btn-delete-file');
...
function fnOutputUpload(message) {
    alert(message);
    fnUnLoading();
}
function fnLoadImage(src) {
    imageEditor.loadImageFromURL(src, 'SampleImage');
    imageEditor.clearUndoStack();
    fnUnLoading();
}
$btnLoad.on('click', function (event) {
    fnLoading();
    window.opener.fnLoadEditImage(fnOutputUpload, fnLoadImage);
});

$btnRefresh.on('click', function () {
    fnLoading();

    imageEditor.loadImageFromURL(window.opener.tmp_src_img_orig, 'SampleImage');
    fnUnLoading();
});
$btnSave.on('click', function () {
    ...
    if (supportingFileAPI) {
        ...
        window.opener.fnSaveEditImage(blob, dataURL, fnOutputUpload);
...
$btnDeleteFile.on('click', function () {
    fnLoading();
    window.opener.fnDeleteImageEdit();
    fnUnLoading();
});
...
imageEditor.loadImageFromURL(window.opener.tmp_src_img, 'SampleImage'); 
imageEditor.loadImageFromURL(window.opener.tmp_src_img_orig, 'SampleImage');